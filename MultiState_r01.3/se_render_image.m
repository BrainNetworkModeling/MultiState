function se_render_image(imgName,u,k,brt,PLTdepth)
...
%=======================================================================
	...
	for i = 1:num,
        ...
        if nargin < 1
            Vi  =  spm_vol(spm_select(1,'image','Select overlay image'));
...
if nargin < 2  || isempty(prevrend.brt)
    ...
    if isfinite(brt)
        ...
        if spm_input('Which colours?','!+1','b',{'RGB','Custom'},[0 1],1)
            for k = 1:num
              if k>3
                col(k,:) = uisetcolor([0 0 0],sprintf('Colour of blob set %d',k));
              else
                col(k,:) = uisetcolor(col(k,:),sprintf('Colour of blob set %d',k));
              end
            end
...
for j=1:length(dat),
    ...
    for i=1:length(rend),
        ...
        if ~isempty(d2)
            ...
            if ~isfinite(brt), msk = find(xyz(3,:) < (z1+30) & xyz(3,:) > (z1-5));
            else, msk = find(xyz(3,:) < (z1+30) & xyz(3,:) > (z1-5)); end
        else
            msk = [];
        end
