...
function [dpRSN,yyRSN] = nets_hierarchy_MS(netmatL,netmatH,DD,sumpics,varargin);    %%%% hierarchical clustering figure
...
%             des = [GM{get(NetFlags.Mask,'value')} '_' PCA{get(NetFlags.PCA,'value')} '_' Glob{get(NetFlags.Glob,'value')} '_' Filt{get(NetFlags.Filt,'value')}];
%         else
%             des = [GM{get(NetFlags.Mask,'value')} '_' PCA{get(NetFlags.PCA,'value')}  '_' Glob{get(NetFlags.Glob,'value')} '_' Filt{get(NetFlags.Filt,'value')} '_' strrep(num2str(NetFlags.Radius,'%2.1f'),'.','')];
%         end
%     end
%     
...
subplot('position',[0 H2 1 1-H2]);
  [dh,dt,dpRSN]=dendrogram(yyRSN,0,'colorthreshold',colour_threshold); set(gca,'ytick',[]); 
  set(gca,'xTickLabel',NetFlags.Label(dpRSN));
  set(dh,'LineWidth',2.5);  % had to remove this for Octave compatibility
subplot('position',[gap 0.04 1-2*gap H1-0.05]); i=dpRSN;
  ...
  xlabel(gca,['reordered pairwise correlation (normalized to maximum r = ' num2str(round(grot1*100)/100) ')']);
  ylabel(gca,['significant pairwise correlations']);
