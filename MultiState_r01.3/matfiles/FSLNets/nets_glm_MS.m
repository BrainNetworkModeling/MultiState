%
% nets_glm(netmats,design_matrix,contrast_matrix,view_output);
...
function [p_uncorrected,p_corrected] = nets_glm_MS(netmats,desi,con,view,des); 
...
system(sprintf('randomise -i %s -o %s -d %s -t %s -x -n 5000',fname,fname,desi,con));
...
if view==1
FIG=  figure('position',[100 100 600*ncon 500]); 
end
...
% close(FIG);
 ...
 nets_boxplots_MS(ts,netmats,groti(1),grotj(1),-1,des);
