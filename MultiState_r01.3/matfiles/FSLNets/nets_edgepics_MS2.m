...
function [netmat] = nets_edgepics(ts,group_maps,meannetmat,net,showN)    %%%% show a snapshot of the kept and rejected components
...
for iii=1:showN
  ...
  title(sprintf('%s <-> %s   r=%.2f',Label{xxx},Label{yyy},netORIG(yyy,xxx)),'Position',[0.5 0.5]);
end
