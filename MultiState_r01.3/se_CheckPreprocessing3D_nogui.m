function se_CheckPreprocessing3D_nogui( list_file, force )
...
if force
    ...
    for sub = 1:numel(subjects)
        ...
        isda = numel(dir(fullfile(subjects{sub},structurals{1},'sm0wrp*.*'))) ...
                + numel(dir(fullfile(subjects{sub},structurals{1},'smwrp*.*'))) ...
                + numel(dir(fullfile(subjects{sub},structurals{1},'m0wrp*.*'))) ...
                + numel(dir(fullfile(subjects{sub},structurals{1},'mwrp*.*')));
        if isda ~= 8
            idx(i) = sub;
            i = i + 1;
        end
