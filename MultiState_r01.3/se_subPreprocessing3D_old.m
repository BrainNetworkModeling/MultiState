function se_subPreprocessing3D(subject,xsub,structurals,conversion)
ms_dir = pwd;
dcm2nii = 0;

subdcmdir = fullfile(subject,'DICOM',structurals);  %   T1 DICOM folder
sub3Ddir  = fullfile(subject,structurals);          %   VBM folder
...
if conversion == 1
    ...
    try; if dcm2nii == 0
        ...
        if numel(fils)>90
            ...
            try movefile(fullfile(sub3Ddir,fx(find([fx.bytes]==max([fx.bytes]))).name),fullfile(sub3Ddir,[xsub '.nii'])); end 
            try delete(fx(find([fx.bytes]~=max([fx.bytes]))).name); end
        end
    end; end

    % --------- dcm2nii conversion by Chris Rorden :: 6 June 2013 -----    
    if numel(dir(fullfile(sub3Ddir,[xsub '.nii']))) ~= 1
...
if numel(dir(fullfile(sub3Ddir,'rp*.nii')))~=2 || numel(dir(fullfile(sub3Ddir,[xsub '_seg8.mat'])))~=1
